...
- **`packages/recozik-web/src/recozik_web/`**: FastAPI backend that consumes `recozik-services`, wires the new auth/quota policies to API tokens, and exposes endpoints such as `/identify/from-path` for GUI/automation clients.
