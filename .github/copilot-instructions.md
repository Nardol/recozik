...
- Frontend (web UI):
  ...
  - `cd packages/recozik-webui && npm run test:e2e` – Playwright E2E suite (includes axe-core accessibility smoke); ensure browsers are installed with `npx playwright install --with-deps chromium`.
