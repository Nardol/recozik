...
| `GET/POST /admin/tokens`                     | Admin-only list/create tokens; payload mirrors `TokenCreateModel` (user_id, display_name, roles, allowed_features, quota_limits). Tokens now reference users via foreign key.    | Configurable hash storage via `auth.db`.                                               |
| `GET /admin/users`                           | Admin-only list all users (`limit`, `offset`). Returns user details (id, username, email, display_name, is_active, roles, allowed_features, quota_limits, created_at).           | Default limit 20, cap 100.                                                             |
| `POST /auth/register`                        | Admin-only create new user. Requires `username`, `email`, `password`, `roles`, `allowed_features`, `quota_limits`. Optional `display_name`.                                      | Password must meet strength requirements (12+ chars, upper, lower, digit, symbol).     |
| `GET /admin/users/{user_id}`                 | Admin-only get user details by ID.                                                                                                                                               | Returns full user record including metadata.                                           |
| `PUT /admin/users/{user_id}`                 | Admin-only update user. Accepts `email`, `display_name`, `is_active`, `roles`, `allowed_features`, `quota_limits`.                                                               | Cannot update username or password via this endpoint.                                  |
| `DELETE /admin/users/{user_id}`              | Admin-only delete user and associated sessions.                                                                                                                                  | Cascades to remove all user sessions; tokens remain but will fail auth.                |
| `POST /admin/users/{user_id}/reset-password` | Admin-only reset user password. Requires `new_password` in body.                                                                                                                 | New password must meet same strength requirements as registration.                     |
| `GET /admin/users/{user_id}/sessions`        | Admin-only list active sessions for a user.                                                                                                                                      | Returns session IDs, creation/expiry timestamps, remember flag.                        |
| `DELETE /admin/users/{user_id}/sessions`     | Admin-only revoke all sessions for a user (force logout).                                                                                                                        | Useful for security incidents or account compromise.                                   |

### Auth & quota model

- **User model**: `User` table (`auth.db`) stores username, email, display_name, password_hash (Argon2id), is_active flag, roles, allowed_features, quota_limits, and created_at timestamp. Admin user is seeded on startup with credentials from `RECOZIK_WEB_ADMIN_USERNAME`/`RECOZIK_WEB_ADMIN_PASSWORD`.
- **Sessions**: UI users log in via `/auth/login` (username/password) receiving HttpOnly cookies (`recozik_session` + `recozik_refresh`) plus CSRF double-submit (`recozik_csrf` cookie + `X-CSRF-Token` header for mutating routes). Session records (`SessionToken` table) link to users via FK and track access/refresh expiry (1h access, 7-30 days refresh depending on "remember me"). Cleanup task runs hourly to purge expired sessions.
- **API tokens**: CLI/automation uses `X-API-Token` header. Static tokens (`RECOZIK_WEB_ADMIN_TOKEN`, optional `RECOZIK_WEB_READONLY_TOKEN`) auto-seed on first request. Dynamic tokens (`TokenRecord` table) reference users via `user_id` FK (integer), stored with hashed token value. Tokens carry display_name, roles, allowed_features, and quota_limits override.
- **Authorization**: `ServiceFeature` values: `identify`, `identify_batch`, `rename`, `audd`, `musicbrainz_enrich`. Tokens carry an `allowed_features` set; `TokenAccessPolicy` rejects unsupported calls with 403. ServiceUser objects use username as `user_id` for job tracking and audit logs.
- **Quotas**: `QuotaScope` keys (`acoustid_lookup`, `musicbrainz_enrich`, `audd_standard_lookup`, `audd_enterprise_lookup`). `InMemoryQuotaPolicy` + optional persistent policy (`persistent_quota.py`) enforce per-user limits and raise 429. Auth endpoints are rate-limited (defaults: 5/min IP).
...
| `RECOZIK_WEB_ADMIN_USERNAME`, `RECOZIK_WEB_ADMIN_PASSWORD`, `RECOZIK_WEB_READONLY_USERNAME`                                      | Username/password for admin user and readonly username (created at startup). |
...
- **Admin tooling**:
  - `UserManager` provides full CRUD for users (create, edit, delete, password reset, session revocation). Shows user table with username, email, roles, features, active status. Only visible when `whoami.roles` contains `admin`.
  - `AdminTokenManager` uses `/admin/tokens` to list/create API tokens. Now requires selecting a user from dropdown (tokens belong to users via FK). Displays token, user, roles, features, and quota limits.
- **API client**: `src/lib/api.ts` centralizes calls (`fetchWhoami`, `uploadJob`, `fetchJobs`, `fetchUsers`, `registerUser`, `updateUser`, `deleteUser`, `adminResetPassword`, `fetchUserSessions`, `revokeUserSessions`, `createToken`, `fetchAdminTokens`). Session-based endpoints use CSRF protection; token endpoints use `X-API-Token` header. `NEXT_PUBLIC_RECOZIK_API_BASE` must point to the backend (default `/api`).
